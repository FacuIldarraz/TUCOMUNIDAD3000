...
//        }
    ...
    public Integer cantidadVotos(String idProyecto) throws ErrorServicio {
        Optional<Proyecto> respuesta = proyectoRepositorio.findById(idProyecto);

        if (respuesta.isPresent()) {

            //       List<Voto> votos = proyectoRepositorio.buscarVotosAFavorProyecto(idProyecto);  AL ARREGLAR LA QUERY SACAR EL COMENTARIO!!!!!!!!!
            //  Integer cantidadVotos = votos.size();
            return 0;
